...
void Menu::dijsktra() {
    ...
    dlist.clone(listRep);
    if (dlist.hasNegativeWeights()) cout<<"UWAGA: Ten graf posiada ujemne krawedzie, algorytm djikstry moze nie dzialac poprawnie"<<endl;
    dlist.start();

    cout<<endl<<endl<<"Reprezentacja macierzowa: "<<endl;
    DijsktraMatrix dmatrix(0, 0, 0);
    dmatrix.clone(matrixRep);
    if (dmatrix.hasNegativeWeights()) cout<<"UWAGA: Ten graf posiada ujemne krawedzie, algorytm djikstry moze nie dzialac poprawnie"<<endl;
    dmatrix.start();
    cout<<endl<<endl;
